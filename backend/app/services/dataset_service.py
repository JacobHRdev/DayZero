...
class DatasetService:
    ...
    def get_account_ids() -> list[str]:
        """Devuelve los account_id disponibles en el dataset, sin duplicados."""
        df = DatasetService.load_dataset()
        if df.empty or "account_id" not in df.columns:
            return []
        return sorted({str(account_id).strip() for account_id in df["account_id"].dropna() if str(account_id).strip()})
