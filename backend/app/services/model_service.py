...
class ModelService:
    ...
    def forecast_custom(self, company: dict) -> Dict[str, Any]:
        """Genera el mismo contrato para una PYME introducida desde la interfaz."""
        balance = float(company["initial_balance"])
        daily_spend = float(company["average_daily_spend"])
        debt_ratio = float(company["debt_balance_ratio"])
        payment_ratio = float(company["payment_balance_ratio"])
        burnout = bool(company.get("burnout", False))
        trend = -daily_spend * (1.0 + debt_ratio)
        sigma = max(150.0, daily_spend * (1.9 + payment_ratio * 10))
        days_until_empty = float(company["days_until_empty"])
        days_to_failure = max(5.0, days_until_empty * 0.62)
        probability = min(0.9, max(0.10, 0.15 + debt_ratio * 0.22 + (0.18 if burnout else 0.0)))
        trajectory = VisualizationService.brownian_path(balance, trend, sigma, days=31, seed=42)
        return {
            "account_id": company["company_name"],
            "fecha_calculo": date.today().isoformat(),
            "dia_observado": 15,
            "parametros": {"B0": round(balance, 2), "mu": round(trend, 2), "sigma": round(sigma, 2), "ventana_dias": 7},
            "prediccion": {
                "dia_esperado_quiebra": round(days_to_failure, 2),
                "intervalo_confianza_90": [round(max(1, days_to_failure - 2.5), 2), round(days_to_failure + 7.8, 2)],
                "intervalo_confianza_50": [round(max(1, days_to_failure - 0.8), 2), round(days_to_failure + 2.3, 2)],
            },
            "riesgo": {
                "fecha_finiquito": date.today().isoformat(),
                "prob_quiebra_antes_finiquito": round(probability, 4),
                "umbral_alerta": 0.35,
                "estado": "riesgo_alto" if probability >= 0.35 else "riesgo_medio",
                "webhook_disparado": probability >= 0.35,
            },
            "serie_saldo": trajectory,
            "gaussiana": VisualizationService.gaussian_density(days=31, expected=days_to_failure, spread=5.0),
        }
